...
fetch(requestURL)
  ...
  .then(function (jsonObject) {
    ...
    const filtermemberships = organizations.filter(members => 
      members.membershiplevel == "Silver" || members.membershiplevel == "Gold"
    );

// Selects one of the random filtered spotlights
spothlights.forEach((spotlight) => {
  const randomnumber = Math.floor(Math.random() * filtermemberships.length);
  const members = filtermemberships[randomnumber];
  filtermemberships.splice(randomnumber, 1);
